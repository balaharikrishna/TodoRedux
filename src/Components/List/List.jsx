...
const List = ({show,changeShowState,getEditListId,getListId,changeShowDelete,clearAllFields,updatedActiveListId}) => {
  const localData = useSelector(state => state.localData)
  const [activeList,setActiveList] = useState(localData ? localData[0]?.id : "");

  useEffect(()=>{
    if(activeList==undefined || activeList == "" || activeList == null){
